...
def legacy_losses(video_length, text_trainable, encoder_hidden_states, noisy_latents, target):
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    def finetune_unet(batch, train_encoder=False):
        ...
        if legacy_train:
            loss = legacy_losses(video_length, text_trainable, encoder_hidden_states, noisy_latents, target)
        else:
            model_pred = unet(noisy_latents, timesteps, encoder_hidden_states=encoder_hidden_states).sample
            loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
